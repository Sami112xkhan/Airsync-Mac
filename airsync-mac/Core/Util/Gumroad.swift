...
class Gumroad {
    ...
    func checkLicenseKeyValidity(key: String, save: Bool, isNewRegistration: Bool) async throws -> Bool {
        ...
        // Local 7-day trial code
        if key == "airsynctrial7" {
            let redemptionKey = "trial_code_redemption_date_airsynctrial7"
            let now = Date()
            
            // Get or set redemption date
            let redemptionDate: Date
            if let storedDate = UserDefaults.standard.object(forKey: redemptionKey) as? Date {
                redemptionDate = storedDate
            } else {
                redemptionDate = now
                UserDefaults.standard.set(redemptionDate, forKey: redemptionKey)
            }
            
            // Calculate expiration (7 days)
            let sevenDays: TimeInterval = 7 * 24 * 60 * 60
            let expirationDate = redemptionDate.addingTimeInterval(sevenDays)
            ...
            if now < expirationDate {
                if save {
                    ...
                    let details = LicenseDetails(
                        key: key,
                        email: "trial7@airsync.local",
                        productName: "AirSync+ (7-Day Trial)",
                        orderNumber: 0,
                        purchaserID: "trial_user",
                        usesCount: 1,
                        price: 0,
                        currency: "usd",
                        saleTimestamp: ISO8601DateFormatter().string(from: redemptionDate),
                        subscriptionCancelledAt: nil,
                        subscriptionEndedAt: ISO8601DateFormatter().string(from: expirationDate),
        ...
        // Magic coupon code bypass
        if key == "airsync100" {
            if save {
                AppState.shared.isPlus = true
                let details = LicenseDetails(
                    key: key,
                    email: "coupon@airsync.local",
                    productName: "AirSync+ (Lifetime Coupon)",
                    orderNumber: 0,
                    purchaserID: "coupon_user",
                    usesCount: 1,
                    price: 0,
                    currency: "usd",
        ...
        // Membership (Monthly/Yearly) and Lifetime IDs
        let membershipProductID = "8uHAUudEWZQqgHMdHa1qvw=="
        let oneTimeProductID = "ry4qK74Ww2B8D9h0mC8O_A=="
